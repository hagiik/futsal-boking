<x-layouts.app :title="__('Tiket Saya')">
    <div class="bg-white dark:bg-zinc-900/50 min-h-screen flex flex-col items-center justify-center p-4 sm:p-6">
        ...
        <header class="w-full max-w-sm text-center mb-6">
            <h1 class="text-2xl font-bold text-lime-500">Tiket Anda</h1>
            <p class="text-gray-800 dark:text-white">Terima kasih telah melakukan pemesanan!</p>
            </header>
            
            <div class="bg-white dark:bg-zinc-800 w-full max-w-sm rounded-2xl shadow-xl overflow-hidden border">
            
                <div>
                    <img class="w-full h-48 object-cover" src="{{ asset('storage/' . $booking->field->image[0]) }}"
                        alt="Foto {{ $booking->field->name }}">
                    </div>
                ...
                <a href="{{ route('profile.showlist') }}"
                    class="text-lime-700 hover:text-lime-800 underline py-4 px-4 rounded-md text-sm">
